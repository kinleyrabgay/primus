...
```
pnpm add @angular/cdk @primeuix/motion @primeuix/styled @primeuix/styles @primeuix/themes @primeuix/utils
...
`@primeuix/*` is the MIT engine (like Angular) — pinned, not vendored.
...
(`@primus/core/icons/baseicon`) via the wildcard.
...
| `@primeng/themes` / `@primeuix/themes` (`definePreset`) | `@primus/theme` (or keep `@primeuix/themes`) |
...
- **`@primus/components/*`** — brand-neutral. Each `*/style/*style.ts` pulls CSS from
  `@primeuix/styles/<component>` — strings of `dt('token.name')`, never literal colors.
...
- **`@primeuix/styled`** — engine: turns preset tokens into CSS variables at runtime.
